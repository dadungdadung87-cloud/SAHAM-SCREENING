...
def hitung_rumus(df):
    """Rumus v5.0 — identik dengan app.py PART 12."""
    ...
    out[1] = df[akumulasi_pro & (df.get('Status Bandar', '') == 'Akumulasi Kuat') & vwap_ok].copy()
    ...
    out[7] = df[(df.get('Prediksi Machine Learning', '') == '🔥 ANOMALI BANDAR (Siap Ledakan)') &
                (df.get('Status Stochastic', '').isin(['Oversold (Jenuh Jual - Peluang)', 'Golden Cross (Awal Bullish)'])) & vwap_ok].copy()
