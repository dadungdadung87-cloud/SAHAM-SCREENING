...
st.markdown("Analisis Tren, Momentum, Volume, dan Aliran Dana Asing secara Real-Time.")
st.markdown("---")
...
if not df_hasil.empty:
    ...
    st.write("") 
    # Baris 2: Filter Skoring & Lanjutan
    ...
    st.write("")
    # 🌟 PERUBAHAN BARU: Baris 3 untuk Papan Perdagangan & Foreign Flow
    col9, col10, col11, col12 = st.columns(4)
    with col9: filter_papan = st.selectbox("🏢 Papan Perdagangan", ["Semua", "Utama & Pengembangan", "Akselerasi", "Pemantauan Khusus"])
    with col10: filter_asing = st.selectbox("🌐 Foreign Flow", ["Semua", "Net Buy", "Net Sell"])
    ...
    if filter_bb != "Semua":
        ...
        else: st.warning("⚠️ Kolom 'Status BB' belum ada di CSV.")

    # 🌟 PERUBAHAN BARU: Logika filter untuk Papan Perdagangan
    if filter_papan != "Semua":
        if "Papan Pencatatan" in df_filtered.columns:
            if filter_papan == "Utama & Pengembangan":
                df_filtered = df_filtered[df_filtered["Papan Pencatatan"].isin(["Utama", "Pengembangan"])]
            else:
                df_filtered = df_filtered[df_filtered["Papan Pencatatan"] == filter_papan]
        else:
            st.warning("⚠️ Kolom 'Papan Pencatatan' belum ada di CSV. Tambahkan kolom ini di update_data.py")

    # 🌟 PERUBAHAN BARU: Logika filter untuk Foreign Flow
    if filter_asing != "Semua":
        if "Foreign Flow" in df_filtered.columns:
            df_filtered = df_filtered[df_filtered["Foreign Flow"] == filter_asing]
        else:
            st.warning("⚠️ Kolom 'Foreign Flow' belum ada di CSV. Tambahkan kolom ini di update_data.py")
    ...
    if not df_filtered.empty:
        ...
        def warna_tabel(val):
            ...
            if isinstance(val, (int, float)):
                ...
                # 🌟 PERUBAHAN BARU: Penambahan warna untuk Net Buy, Papan Akselerasi & FCA
                if val in ["Positif", "Uptrend", "BELI", "Breakout Upper", "Bottom Rebound", "Net Buy", "Utama"]: 
                    style = 'color: #22c55e; font-weight: 600;'
                elif val in ["Negatif", "Downtrend", "WAIT & SEE", "Net Sell", "Pemantauan Khusus", "Akselerasi"]: 
                    style = 'color: #ef4444; font-weight: 600;'
                elif val == "> 1 Miliar" or val == "Pengembangan": 
                    style = 'color: #3b82f6; font-weight: 600;'
                elif val == "Squeeze": 
                    style = 'color: #fbbf24; font-weight: 600;'
            return style

        # 🌟 PERUBAHAN BARU: Tambahkan kolom baru ke daftar pewarnaan
        kolom_berwarna = ["Change (%)", "Momentum", "MA Signal", "Rekomendasi", "Likuiditas", "Status BB", "Papan Pencatatan", "Foreign Flow"]
...
if len(watchlist_terbaru) > 0 and not df_hasil.empty:
    ...
    def pewarnaan_tabel_watchlist(val):
        if isinstance(val, str):
            # 🌟 PERUBAHAN BARU: Tambahkan ke aturan warna Watchlist
            if val in ["Uptrend", "Positif", "BELI", "Tembus MA20", "Breakout Upper", "Bottom Rebound", "Net Buy"]:
                return "color: #4ade80; font-weight: bold;" 
            elif val in ["Downtrend", "Negatif", "Net Sell", "Pemantauan Khusus", "Akselerasi"]:
